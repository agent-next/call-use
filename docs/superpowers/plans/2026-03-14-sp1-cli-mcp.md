...
- cancel: sends cancel command via data channel (topic: backend-commands)
- result: retrieves final CallOutcome from room metadata after call ends (state: ended)

The agent itself creates the SIP participant once dispatched. This avoids the stdio
MCP single-request-at-a-time bottleneck.
"""
...
async def _do_dial(
    ...
    """Internal: dispatch a call via LiveKit agent_dispatch and return immediately with task_id.
