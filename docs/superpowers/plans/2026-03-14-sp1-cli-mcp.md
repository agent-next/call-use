...
async def test_do_dial_returns_task_id(MockLiveKitAPI):
    """dial tool dispatches agent and returns task_id immediately (non-blocking)."""
    mock_api = AsyncMock()
    ...
    # Verify agent was dispatched (not SIP participant created directly)
...
async def test_do_dial_with_user_info(MockLiveKitAPI):
    """dial tool passes user_info as agent dispatch metadata."""
    mock_api = AsyncMock()
    mock_api.room.create_room.return_value = MagicMock()
    mock_api.agent_dispatch.create_dispatch.return_value = MagicMock()
    MockLiveKitAPI.return_value.__aenter__ = AsyncMock(return_value=mock_api)
    ...
    assert "task_id" in result
    # Verify user_info was included in dispatch metadata
    dispatch_call = mock_api.agent_dispatch.create_dispatch.call_args
    request_obj = dispatch_call[0][0]  # first positional arg: CreateAgentDispatchRequest
...
async def _do_dial(
    ...
    Creates a LiveKit room and dispatches the call-use agent into it. The agent
    itself creates the SIP participant (see agent.py line 535-546).
    Does NOT block for call completion.
    """
    ...
    async with LiveKitAPI() as api:
        ...
        # Dispatch agent — the agent will create the SIP participant itself
        metadata = json.dumps({
            "phone_number": phone,
            "instructions": instructions,
...
async def _do_result(task_id: str) -> dict:
    ...
    async with LiveKitAPI() as api:
        ...
        return {"task_id": task_id, "status": "in_progress", "state": metadata.get("state", "unknown")}
...
async def cancel(task_id: str) -> str:
    ...
    try:
        async with LiveKitAPI() as api:
            # Send cancel command via data channel — matches agent._on_data_received()
            # which listens on topic "backend-commands" for {"type": "cancel"}
            await api.room.send_data(lk_proto.SendDataRequest(
                room=task_id,
                data=json.dumps({"type": "cancel"}).encode(),
                kind=lk_proto.DataPacket.Kind.RELIABLE,
